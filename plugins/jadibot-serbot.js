...
// ✨ MODIFICACIÓN PRINCIPAL: Usar límite dinámico
const maxSubBots = global.maxSubBots || 25 // Límite personalizable o 25 por defecto
if (subBotsCount >= maxSubBots) {
  return m.reply(`${emoji2} No se han encontrado espacios para *Sub-Bots* disponibles.\n\n📊 *Estado actual:*\n• Sub-Bots activos: ${subBotsCount}/${maxSubBots}\n• Espacios disponibles: 0\n\n💡 Un owner puede aumentar el límite con: ${usedPrefix}limitbot <número>`)
}
...
// ✨ MENSAJE MEJORADO con información del límite
const currentSubBots = global.conns.filter((conn) => conn.user && conn.ws.socket && conn.ws.socket.readyState !== ws.CLOSED).length
const maxSubBots = global.maxSubBots || 25
const spacesLeft = maxSubBots - currentSubBots

m?.chat ? await conn.sendMessage(m.chat, {
  text: args[0] 
    ? `@${m.sender.split('@')[0]}, ya estás conectado, leyendo mensajes entrantes...\n\n📊 *Estado Sub-Bots:* ${currentSubBots}/${maxSubBots}\n🆓 *Espacios disponibles:* ${spacesLeft}` 
    : `@${m.sender.split('@')[0]}, genial ya eres parte de nuestra familia de Sub-Bots.\n\n📊 *Sub-Bots activos:* ${currentSubBots}/${maxSubBots}\n🆓 *Espacios disponibles:* ${spacesLeft}`, 
  mentions: [m.sender]
}, { quoted: m }) : ''
