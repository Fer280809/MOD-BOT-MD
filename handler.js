    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
          ...
          // BAN COMPACTO Y CON EMOJIS
          if (m.text && user.banned && !isROwner) {
            m.reply(`🚫 *Estás BANEADO/A*\n${user.bannedReason ? `🔹 Motivo: _${user.bannedReason}_` : ''}\n\nSi crees que es un error, contacta al Owner.`)
            user.antispam++
            return
          }
        ...
        if (!isPrems && plugin.coin && global.db.data.users[m.sender].coin < plugin.coin * 1) {
          conn.reply(m.chat, `💰 *Te quedaste sin monedas!*`, m)
          continue
        }
        // NIVEL INSUFICIENTE CON EMOJIS Y COMPACTO
        if (plugin.level > _user.level) {
          conn.reply(m.chat,
            `🆙 *Nivel insuficiente*\n🔑 Requerido: *${plugin.level}* | Tú: *${_user.level}*\n\n🎯 Sube de nivel: *${usedPrefix}levelup*`, m)
          continue
        }
        let extra = {
          match,
        ...
        try {
          ...
          if (m.coin)
            conn.reply(m.chat, `💸 Usaste ${+m.coin} monedas.`, m)
        }
        break
      }
    }
...
// MENSAJES DE ERROR/RESTRICCIÓN MEJORADOS, SOLO ESTA PARTE EDITADA
global.dfail = (type, m, usedPrefix, command, conn) => {
  ...
  const msg = {
    rowner: `👑 *Solo el CREADOR puede usar:* _${comando}_`,
    owner: `👤 *Solo OWNER puede usar:* _${comando}_`,
    mods: `🛡️ *Solo MODERADORES pueden usar:* _${comando}_`,
    premium: `💎 *Solo usuarios PREMIUM pueden usar:* _${comando}_`,
    group: `👥 *Solo en GRUPOS se permite:* _${comando}_`,
    private: `📱 *Solo en PRIVADO se permite:* _${comando}_`,
    admin: `🔒 *Solo ADMIN del grupo puede usar:* _${comando}_`,
    botAdmin: `🤖 *Hazme admin para usar:* _${comando}_`,
    unreg: `📝 *Debes registrarte para usar:* _${comando}_\n\nUsa: *${usedPrefix || '#'}${verifyaleatorio} ${user2}.${edadaleatoria}*`,
    restrict: `⚠️ *Función desactivada por el Owner*`
  }[type];
